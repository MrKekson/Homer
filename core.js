...
app.controller('HomerRoot', ['$scope','$http', function($scope, $http) {
  
  $scope.homer = {  $scope : false, 
                    message : "Doh!",
                 };



  $scope.updateActiveBtn = function(){ 
    $http.get("/getfutes")
      .then(function(response) {
        $scope.homer.active = response.data.futes;
        $scope.homer.heating = response.data.futes01;
        $scope.homer.active ? $scope.homer.activeBtn = "Ki" :  $scope.homer.activeBtn = "Be";

    });
  }                 

  $scope.updateActiveBtn();

  $scope.ToggleActive = function (  ){

    if ($scope.homer.active){
          $http.get("/setheating/off")
          .then(function(response) {
            $scope.names = response.data.records;
            $scope.updateActiveBtn();
          });
    }else{
          $http.get("/setheating/on")
          .then(function(response) {
            $scope.names = response.data.records;
            $scope.updateActiveBtn();
          });
    ...
    $scope.homer.active = !$scope.homer.active;
...
/*  https://github.com/seiyria/angular-bootstrap-slider/blob/master/slider.js */
